import React from 'react'
import './index.css';
import {useNavigate} from "react-router-dom"

const Home = () => {

  const navigate = useNavigate();

  return (
    <div
      className='home'
    >
      <div className="navHome">
        <div 
          className="logo"   
          onClick={()=>{
            navigate(0);
          }}
        >
          <span className="spanLogo1" >
            PEST
          </span>
          <span className="spanLogo2" >
            &nbsp;ID
          </span> 
        </div>
        <div className="navCont">
          <button 
            onClick={()=>{
              navigate(0);
            }}
            className="navContBtn">
            Acceuil
          </button>
          <button 
            onClick={()=>{
              navigate("/about");
            }}
            className="navContBtn" >
            À propos
          </button>
          <button 
            onClick={()=>{
              navigate("/contact");
            }}
            className="navContBtn" >
            Contact
          </button>
        </div>
        <div className="btnCont">
          <button 
            onClick={()=>{
              navigate("/login");
            }}
            className="btnConexion" >
            Connexion
          </button>
          <button 
            onClick={()=>{
              navigate("/become-member");
            }}
            className="btnCréerCpt" >
            Créer un compte
          </button>
        </div>
      </div>
      <div className="contHome">
          <div className="row1">
            Cultivez l'Excellence
          </div>
          <div className="row1 row2 ">
            avec Précision
          </div>
          <div className="row3">
            Optimisez vos récoltes avec des prédictions 
          </div>
          <div className="row4">
            fiables pour <span>des décisions agricoles rentables.</span>
          </div>
          <div className="row5">
            <button 
              onClick={()=>{
                navigate("/become-member");
              }}
              className='devenirM ' 
            >
              Devenir Membre &nbsp;
              <svg stroke="white" fill="white" stroke-width="0" viewBox="0 0 24 24" class="ml-2 text-white shrink-0" height="25" width="25" xmlns="http://www.w3.org/2000/svg"><path d="M4.035 15.479A3.976 3.976 0 0 0 4 16c0 2.378 2.138 4.284 4.521 3.964C9.214 21.198 10.534 22 12 22s2.786-.802 3.479-2.036C17.857 20.284 20 18.378 20 16c0-.173-.012-.347-.035-.521C21.198 14.786 22 13.465 22 12s-.802-2.786-2.035-3.479C19.988 8.347 20 8.173 20 8c0-2.378-2.143-4.288-4.521-3.964C14.786 2.802 13.466 2 12 2s-2.786.802-3.479 2.036C6.138 3.712 4 5.622 4 8c0 .173.012.347.035.521C2.802 9.214 2 10.535 2 12s.802 2.786 2.035 3.479zm1.442-5.403 1.102-.293-.434-1.053A1.932 1.932 0 0 1 6 8c0-1.103.897-2 2-2 .247 0 .499.05.73.145l1.054.434.293-1.102a1.99 1.99 0 0 1 3.846 0l.293 1.102 1.054-.434C15.501 6.05 15.753 6 16 6c1.103 0 2 .897 2 2 0 .247-.05.5-.145.73l-.434 1.053 1.102.293a1.993 1.993 0 0 1 0 3.848l-1.102.293.434 1.053c.095.23.145.483.145.73 0 1.103-.897 2-2 2-.247 0-.499-.05-.73-.145l-1.054-.434-.293 1.102a1.99 1.99 0 0 1-3.846 0l-.293-1.102-1.054.434A1.935 1.935 0 0 1 8 18c-1.103 0-2-.897-2-2 0-.247.05-.5.145-.73l.434-1.053-1.102-.293a1.993 1.993 0 0 1 0-3.848z"></path><path d="m15.742 10.71-1.408-1.42-3.331 3.299-1.296-1.296-1.414 1.414 2.704 2.704z"></path></svg>
            </button>
            <span>
              Ou bien
            </span>
            <button 
              className='reserverD '
              onClick={()=>{
                navigate("/appointment/schedule");
              }}
            >
              Réserver une Démo
            </button>
          </div>
      </div>
      <div className="footer">
        Une solution signée&nbsp;
        <a href="https://pcs-agri.com/" target='_blank' >PCS AGRI</a>
        &nbsp;© 2024
      </div>
    </div>
  )
}

export default Home